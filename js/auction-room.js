(function () {
  ...
  function renderValuation() {
    ...
    if(!player){$('selected').textContent='Select a player from the board.';['intrinsic','leagueprice','currentbid','maxbid','surplus','pointgain'].forEach((id)=>$(id).textContent='—');$('decision').textContent='';$('nomination').textContent='';$('why').textContent='';if($('auctionComparable'))$('auctionComparable').textContent='Select a player to compare the next option.';return}
    ...
    if($('auctionComparable'))$('auctionComparable').innerHTML=a.nextComparable?`<strong>${esc(a.nextComparable.player.name)} · ${esc(a.nextComparable.player.position)}</strong><span>${a.nextComparable.sameTier?'Same tier':'Next tier'} · saves about ${money(Math.max(0,a.current-a.nextComparable.price))} · ${money(a.nextComparable.price)} format value</span>`:'<strong>No close substitute</strong><span>The position falls off sharply after this player.</span>';
    $('why').textContent=`Format value reflects ${state.auction.league.teams} teams and ${formatLabel(state.auction.league)}. Starter-value gain ${a.pointGain.toFixed(1)}; roster need ${Math.round(a.need)}/100; scarcity ${Math.round(a.scarcity.scarcity)}/100; room ${roomInflation().toFixed(2)}×. Price confidence ${a.priceConfidence.toLowerCase()} (${a.priceEvidence} matched position/tier observations); ${a.priceConfidence==='UNMODELED'?'premium capped at 12% above format value until league evidence exists. ':''}Maximum bid preserves ${Math.max(0,state.auction.teams[state.auction.userTeamId].slotsLeft-1)} future minimum bids.`;
  ...
  function renderLot() {
    ...
    if(awaitingUser&&nomination){$('bidLot').textContent=`Bid ${money(nomination.currentBid+auction.config.minBid)}`;}else $('bidLot').textContent='Bid';
  }
